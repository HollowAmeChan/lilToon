...
| 家族宏 | `LIL_LIQUID` |
| 技术开关 | `LIL_LIQUID_WAVE` / `LIL_LIQUID_UNDERLAY` |
| 属性前缀 | `_Liquid*` |
...
```shaderlab
        ...
        // Liquid - Specular / Underlay
                        _LiquidSpecularStrength  ("sLiquidSpecularStrength", Range(0, 1)) = 1
                        _LiquidUnderlayTex       ("sLiquidUnderlayTex", 2D) = "black" {}
                        _LiquidUnderlayColor     ("sColor", Color) = (1, 1, 1, 1)
                        _LiquidUnderlayScroll    ("sLiquidUnderlayScroll", Vector) = (0.08, 0.03, 0, 0)
```

**两条只在液体家族成立的约定：**

- `_LiquidUnderlayTex` 要 **UV 滚动**，所以**不能**加 `[NoScaleOffset]`
  （加了就没有 `_LiquidUnderlayTex_ST` 可用），C# 侧 `lilMaterialProperty(name, ...)` 的
  second 参数也必须相应为 `false`。
- `_LiquidOffset` 是 `Range(-1, 1)` 而不是 `Float`：
  它的用途是"脚本传入的上下限 ±1 的垂直偏移量"，用 `Float` 的话 Inspector 会允许
  手滑输成 100，液面直接飞出容器。
...
正面的液面与透过切面的洞看到的内部才会落在同一处（否则洞口和液面会错位）。
...
- 比较基准是**物体空间**的 `fd.positionOS`，所以 `_LiquidSurfaceWidth` 的量纲是
  "物体空间的米"，跟着模型一起缩放，跨分辨率稳定。参考实现用的是 clip 空间
  `positionCS.y / positionCS.w` + `step(..., 0.5)`，那套在分辨率变化时要重调，没有采用。

### 4.4 液面画法：只有一条软过渡通路（已定稿）

> **本节的结论是经过三轮试错后收敛的，不要再往这里加模式开关。**
> 早期设计过 `_LiquidSurfaceMode` 三档（`Liquid` / `LiquidCap` / `Off`），
> 以及配套的 `LIL_LIQUID_CAP` 封顶层。三条封顶路线全部被否决（理由见 §5 L2），
> 参数和分支已在 rev8 全部删除。**现在液面只有一条通路。**
...
```hlsl
// lil_liquid_level.hlsl
float lilLiquidCalcSoft(float3 N)
{
    // |dot|：0 = 像素面与液面平行（竖直面），1 = 像素面贴着液面（朝上/朝下）
...
{
    float width = lilLiquidCalcSoft(N);
    // 关键不变式：alpha(d == 0) 必须恰好 == 0.5 == _Cutoff
...
- **`alpha(d=0) == 0.5 == _Cutoff` 是硬约束。** 写成 `saturate(d / width * 0.5 + 0.5)`
  而不是 `saturate(d / width)`，就是因为前者在 d=0 处正好落在 cutout 阈值上，
  液面位置不随 `_LiquidSurfaceWidth` 漂移。改成别的写法必须重新验算这一条。
- **`abs()` 而不是裸 `dot()`**：容器竖直时 `n≈0.5`（因为竖直面的法线与 up 垂直），
  但容器倒过来时 `fd.N` 可能整体反向，`abs` 让软边宽度在两种朝向下对称。
- **液面高度与 `_LiquidFill` 的关系是"切的平面"**，网格必须有足够的纵向细分，
  否则切边是台阶。参考实现的 `model.fbx` 就是照着这个前提做的；
  我们实测的胶囊体（纵向 32 段）在切面附近已经足够平滑。
- **正面/背面用同一条 `alpha`**，不做区分。切面留下的那个洞（`fd.facing < 0`）
  由 lilToon 现有的 `_BackfaceColor` 自动染色——这就是"液体正背面如何渲染"的完整答案，
  不需要新写分支。想给内部一个独立颜色，就给 `_BackfaceColor` 一个偏色的 HDR 值
  （**注意它的默认 alpha 是 0，不改就没效果**）。
- 想让液面有独立颜色，也可以用 `_UseMain2ndTex` + `_Main2ndTex_Cull`（cull 掉正面或背面）。
...
> **已交付 = L0 + L1，L2 被否决，L3/L4 不做。** 下面保留被否决项的记录，
> 是为了以后有人再想"给液面加个盖子"时先读到结论。

### L0 基础设施（已交付）
- `Hidden/lilToonLiquid` 家族骨架（6 个文件）
- `lil_liquid.hlsl` + `lil_liquid_level.hlsl`：液面高度场 + 单条软切 + 正背面分派
...
- `_LiquidSurfaceWidth` 的软切改善斜视角（§4.4 的单条通路）
- 世界空间波（`_LiquidWaveSpace=1`）与物体空间波的对比
- 液面高光：`_LiquidSpecularStrength` 把 `fd.reflectionN` / `fd.matcapN`
  混向液面平面法线（见 `lilLiquidApplySpecular()` 的注释与 §11 坑 13）
- 液体内部常态流动层：`_LiquidUnderlayTex` + `_LiquidUnderlayColor` + `_LiquidUnderlayScroll`，
  只作用于 `fd.facing < 0`（透过切面的洞看到的那片内部）

### L2 封顶（**已否决，不实现**）

切一个闭合体积必然在切面处留一个洞——这是拓扑问题，纯着色填不上。
为了让那个洞看起来像"液面"，先后试过三条路，全部否决：

| 方案 | 做法 | 否决理由 |
|---|---|---|
| 专用液面圆盘网格 | 跟着 `_LiquidFill` 移动一块单独的 disc mesh | 需要为每个容器手工量半径、多一个 renderer、多一份材质；容器非圆截面（方瓶、异形瓶）直接不成立 |
| 顶点钳制几何切面 | 在顶点着色器里把液面以上的顶点压到液面上，让洞自己闭合 | **实测不成立**：对凸的闭合网格，沿法线把切面以上的顶点压到切面上会留一圈环状孔。胶囊体 R=1、切在 y=1.5 时，压完得到 r_max=0.866，环宽 0.134 —— 洞还在，只是变小了。而且它把顶点位移和 lilToon 的 outliner / dissolve 通道搅在一起 |
| 模板 + 全屏补面 | 模板标记洞口 + 全屏 pass 填液体颜色 | 要额外的全屏 pass 和 stencil 状态，在 URP 下与 lilToon 的现有 pass 结构冲突；收益（一块纯色）远小于复杂度 |

**最终方案：不封顶。** 洞就是 `fd.facing < 0` 的那片液体内部，交给 lilToon 现成的
`_BackfaceColor` 染色，外加可选的 `_LiquidUnderlayTex` 常态流动层。
参考实现（`41水瓶/Liquid.shader`）本来就是这么做的——它那个"盖子"其实也只是
背面那一圈被 `_TopColor` 染色的内壁，并不是真的封顶几何。

> 用户验收原话：「我用了这个办法其实很好，你之前那三路切换其实全都可以删了
> 直接默认就不管封顶算了」。对应提交 `refactor(液体): 删除 _LiquidSurfaceMode 三档`。
...
## 6. 一次性共享文件改动（比 hair 少得多）

hair 当时改了 6 个共享 include（CBUFFER + TEXCOORD + TBN + 家族列举）。
液体的 CBUFFER 只需要一张贴图的 float4 组，**新属性全在自己家族的 `#if defined(LIL_LIQUID)` 块里**，
所以只需要把 `positionOSdissolve` 这个现成通道借过来。实测改动了下面这些：

| 文件 | 位置 | 改动 |
|---|---|---|
| `lil_common_input.hlsl` | `_HairLobe*` 那一组之后 | `#if defined(LIL_LIQUID)` 块：19 个成员（**每个声明都要分号**）+ `TEXTURE2D(_LiquidUnderlayTex);`（**必须在 CBUFFER 外面**，见 §11 坑 5） |
| `lil_common_input_base.hlsl` | 同上 | **逐字一致**的 CBUFFER 那一块（19 个成员）；但**没有** `TEXTURE2D(_LiquidUnderlayTex);` —— 贴图声明只归 `lil_common_input.hlsl` 管 |
| `lil_common_input_opt.hlsl` | 同上 | 同 `_base`，逐字一致 |
...
| 7 | `lilInspector/lilMaterialProperties.cs` | L406 后 | 19 个 `lilMaterialProperty(..., PropertyBlock.Liquid)` 声明（`_UseLiquid` + 18 个 `_Liquid*`） |
...
> 下面是**实际交付的形态**（rev8）。`lil_liquid_level.hlsl` 里是各段数学，
> 这里只做编排：算 d → 算 alpha → 写进 `fd.col.a`。
...
void lilLiquid(inout lilFragData fd LIL_SAMP_IN_FUNC(samp))
{
    ...
    lilLiquidApplySpecular(fd);                   // 高光：只改 reflectionN / matcapN

    float d     = lilLiquidSurfaceDistance(fd);   // >0 液面以下
    float alpha = lilLiquidSurfaceMask(fd, d);    // 单条软切，alpha(d=0) == 0.5
...
调用点：`lil_pass_forward_liquid.hlsl` 的 `frag` 里，**在 Normal 段之后、
Alpha（cutout）段之前**。

> ⚠️ **这个位置是踩过坑才定下来的。** 最初照抄 hair 的 `lilHairSpecular()` 调用点，
> 把 `lilLiquid(fd)` 放在了 frag 末尾 —— 那时 `fd.col.a` 已经被 cutout 段消费过了
> （`fd.col.a = saturate((fd.col.a - _Cutoff) / max(fwidth(fd.col.a), 0.0001) + 0.5); if(fd.col.a == 0) discard;`），
> 再写 alpha 完全无效，症状是"液面高度怎么调都没反应 / 永远满"。
> **切面类的模块必须写在 cutout 段之前。**
...
| **2** | 液面切面画法 | **只有一条软切通路**（`saturate(d/width*0.5+0.5)`，见 §4.4 已定稿） | ~~模式 0/1/2 三档~~ 已删除；封顶三条路全部否决，见 §5 L2 |
| **3** | 背面液面用 lilToon 的 `_BackfaceColor` 还是新属性 | **复用 `_BackfaceColor`**（已在 CBUFFER，零新增） | 新加独立 HDR 属性，但要加属性 + CBUFFER + GUI + 本地化 |
| **4** | L0 要不要一开始就做 `HO_*` 5 个 pass | **保留全部 11 个 pass**（照 hair 的结论，MB/GB 管线在用） | 照 gem 砍到 8 个 |
...
| 你的意见（原话） | 落地 |
|---|---|
| 「液面高度还得加一个高度偏移输入量（+-1 上下限）来让脚本传入」 | `_LiquidOffset`，`Range(-1, 1)`，直接加到 `d` 上 |
| 「脚本写振幅，手动也能调振幅，两者合成最终的值」 + 「不要叫这个名字，直接叫乘数」 | `_LiquidWaveAmpMul`（显示名「波纹振幅乘数」），`lilLiquidWaveAmplitude() = _LiquidWaveAmp * _LiquidWaveAmpMul` |
| 「一切都太麻烦了别加东西了……最直接的办法是拿到盖子上面这个区域乘一个颜色算了」 / 「那三路切换其实全都可以删了」 | 删除 `_LiquidSurfaceMode` + `LIL_LIQUID_CAP`，切成"切面 + `_BackfaceColor` 染色 + 可选 `_LiquidUnderlayTex`"，见 §4.4 / §5 L2 |

**为什么是"乘数"而不是"加成"**：`_LiquidWaveAmp` 默认 **0**，是一个刻意的**闸门** ——
美术不主动抬它，脚本就掀不起浪；`_LiquidWaveAmpMul` 默认 **1** 是乘法单位元，
所以老材质的观感完全不变。加成式做不到这一点（0 基准下加成照样出浪）。
...
| 液体属性 | 19 个全部进了 `Properties` 与 C# 声明（`_UseLiquid` + 18 个 `_Liquid*`；`_LiquidSurfaceMode` 已在 rev8 删除） |
| **PO 结构** | 5 个 `.po` 全部 `violations=0`、无 BOM、纯 CRLF ✅ |
| 本地化覆盖 | 5 语言对 `DefaultLiquid.lilblock` 的 display key 全覆盖（仅 `sCullModes` / `sLiquidWaveSpaces` 缺席，二者由 `InitializeLabels` 的 `BuildParams` 提供，属预期） |
...
**后续（`BREAK_URP` 实机验证）补记：**

- 液面高度、`_LiquidSurfaceWidth` 手感、`Wobble.cs` 的直连效果**都已在实机验证通过**
  （测试资产：`Assets/Hollow/土豆/PTP_M_WaterBottleGem.mat` 挂在一个正方体上）。
- 实机暴露的两个问题及结论：
  1. **切面的洞** —— 见 §5 L2，最终不封顶，交给 `_BackfaceColor`。
  2. **`fd.positionOS` 断链 + 调用点错位** —— 见坑 0 与坑 0.5，这是"液面永远满"的真凶。
...
5. **CBUFFER 里每个声明都必须有分号。**（本次实测：某条 `uint _LiquidXxx` 漏了 `;`）
   ...
   **而且液体有 3 份 CBUFFER（`lil_common_input.hlsl` / `_base` / `_opt`），改一处必须三处同步**，
   否则不同 SRP / 精度档下行为不一致。改完用 `git diff` 三份对照着看。
6. **`sLiquidWaveSpaces` 要在三处登记**（`[lilEnum]` 不走 `.po`）：
   ① `lilLanguageManager` 的 `public static string` 字段
   ② 同文件 `InitializeLabels()` 里的 `BuildParams(...)` 赋值
   ③ `lilEditorVariables` 的转发属性
   漏①就是 `CS0103: The name 'sLiquidWaveSpaces' does not exist in the current context`。
   漏③不会报错，但面板上会显示成生的 key。
...
13. **`_LiquidSpecularStrength` 要生效，必须把两个 Normal Strength 拉到 0。**（实测）
    `lilCalcSpecular():1335` 与 `lilReflection():1497` 都是
    `N = lerp(fd.origN, <专用法线>, _XxxNormalStrength)`，
    默认 `_SpecularNormalStrength = _ReflectionNormalStrength = 1`，
    会把容器壁的**水平**法线混回来 → 液面上出现一圈"贴着容器壁"的错高光。
    面板上把这俩拉到 0，液面法线才完全接管。
14. **驱动端给的是「度」，且 `_LiquidTiltX/Z` 的语义是"哪一侧抬高"，不是旋转轴。**（见 §4.5）
...
15. **切面处的洞不要试图用几何补。**（三轮试错的结论，详见 §5 L2）
    顶点钳制那套尤其有迷惑性——它"看起来"能把洞口收紧，实测对凸闭合网格仍留环状孔
    （胶囊 R=1、切在 y=1.5 → 压完 r_max=0.866，环宽 0.134）。
    正确做法是接受洞，用 `_BackfaceColor` + 可选的 `_LiquidUnderlayTex` 顶替。
16. **`_BackfaceColor` 的默认 alpha 是 0**，不改就等于没开背面染色。
    给个 `(0.08, 0.18, 0.22, 0.9)` 这种偏色值即可，不需要新写任何分支。
17. `.shader` 是生成物，**不手改、不提交**；改完 `.lilblock` 必须回 Settings 页点 `Apply`。
18. **不要用 PowerShell 的字符串替换去改共享 include。**（本次实测）
...
19. **⚠️ 同一个文件里有"两个 up"，它们是转置关系，混用会在 45° 出错。**（实测）
...
### 12.1 交付后的修订单（按时间顺序，都是实机反馈驱动的）

| 提交 / 主题 | 内容 |
|---|---|
| `feat(液体)` 家族本体 + Editor 接线 + 文档 | 初始交付 |
| `fix(液体)` 位置通道断链 | §11 坑 0 的两个断点 |
| `fix(液体)` 调用点提前到 cutout 之前 | §11 坑 0.5 |
| `fix(液体)` 两轴倾角 + 独立偏移 | §9.1，与参考实现的最大分歧 |
| `feat(液体)` 波纹振幅乘数 | `_LiquidWaveAmpMul`，最终值 = 基准 × 乘数 |
| `feat(液体)` 液面高光 + 内部流动层 | `_LiquidSpecularStrength` / `_LiquidUnderlayTex` |
| `refactor(液体)` 删除 `_LiquidSurfaceMode` 三档 | §5 L2，封顶不做了，改用 `_BackfaceColor` |
| `docs(液体)` | 本文档同步到 rev8 |

**每次改 `lil_liquid*.hlsl` 的「行为」都要把 `LIL_LIQUID_REV` +1**（当前 **8**；纯注释不用），
它是确认"Unity 跑的是不是刚改的那版"的唯一手段：
