...
```
┌ 工具条 ─────────────────────────────────────────────────────────────────────┐
│ [含未激活：开] [刷新] [从场景同步] [全选] [清空] [展开] [收起]    [搜索…]      │
...
│     ☑ Skirt       │────────────────────────────────│ ── 阴影 ──              │
│     ☐ Coat        │ 分组：Hidden/lilToonCutout ×12 │                         │
│                   │   12:03 _AODarkStrength 0.5→0.7 ×11                     │
│                   │   12:03 ⚠ 跳过 1 个（没有该属性）：Cloth             │
└───────────────────┴────────────────────────────────┴─────────────────────────┘
（材质表与日志之间是横向分隔条，可上下拖动、双击复位；日志没有头栏、永远展开，
  清空/复制/选中情况都在右键菜单里；右栏只在多分组时才标一下组名）
```

- **左栏**：当前场景的 GameObject 层级树，tri-state 复选（父级勾选 = 递归选中其下所有"有 lilToon 材质的物体"）；只显示到"有材质的节点 + 其祖先链"，避免整棵场景树噪声。
- **中栏上半**：去重后的 lilToon 材质列表（一行一个材质**资产**）。列按"从右往左排、名字列吃剩下的"分配：
  `[勾选 20][材质名 自适应][Shader 176 可选][×N 40][变体 34 可选][定位图标 22]`
  - **Shader 列只在栏宽 ≥ 340 时出现**（中栏本来就窄，窄栏时把宽度让给名字；拖宽中栏它自己会出来）
  - **变体列只在列表里真有 Material Variant 时才占位**，否则宽度还给名字
  - 列位置由一份 `Columns` 计算同时供表头和数据行使用，不再两边各算一套
  - 最右是**定位图标**（放大镜，`d_Search Icon`）：点一下在 Project 里选中并 Ping 该材质文件；**内嵌在 Prefab / FBX 里的材质**（`AssetDatabase.GetAssetPath` 返回宿主资产）定位到宿主资产，否则 Project 里根本没有它的条目。图标无边框、hover 才加一层浅底 + 手型光标 + tooltip；和勾选框一样走手动命中（不做成 `GUI.Button`）—— 行是虚拟化的，滚动时控件 ID 会漂移
- **中栏下半（日志控制台）**：顶部固定一行**分组构成**（`分组：Hidden/lilToonCutout ×12`，多组时列出每组），下面是改动记录 + 诊断。独立可拖（横向分隔条调高度，双击复位，高度记进 `EditorPrefs`）。**没有头栏，永远展开**；选中情况/影响面、清空/复制都在右键菜单里。见 §4.4。
- **右栏**：选中材质集的输入值编辑区，按 lilToon 的属性分组折叠显示；**右栏自身永远展开**，且**只有一个属性组时不画任何组标题**（"shader ×N 个材质"统一在日志顶部）；没选材质时右栏保持空白，不做提醒。
...
6. **区间勾选（Shift+点击）**：普通点击 = 切换这一行，并把该行设为**锚点**；**Shift+点击 = 把"锚点行 → 点击行"这一段统一刷成点击行的新状态**（中栏按材质逐个刷；左栏按"每个节点刷它整棵子树"，与单点行为一致）。锚点按**对象引用**记而不是行号 —— 排序、过滤、折叠展开都会让行号变，对象引用不会；锚点被过滤掉时退化成普通切换。锚点只在普通点击时移动，Shift+点击不动它，所以可以连续 Shift+点击来伸缩这一段。
...
- **顶部固定一行"分组构成"**：`分组：Hidden/lilToonCutout ×12`；跨组时列出每组（`分组 2 组：A ×9　·　B ×3`）。这行原来是右栏顶部的组标题（`shader ×N 个材质`），挪到这里之后右栏只剩属性列表。没有选中材质时这行不显示。
...
工具条的开关（`含未激活`）沿用隔壁"激活 = 绿色高亮"的配色，并且**标签上直接写开/关**（`含未激活：开` / `含未激活：关`）—— 只靠 `toolbarButton` 的按下态在深色皮肤下几乎看不出来。
