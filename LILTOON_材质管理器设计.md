...
- **中栏上半**：去重后的 lilToon 材质列表（一行一个材质**资产**）。列：勾选、材质名、Shader 变体、使用次数 `×N`、是否 Material Variant，以及最右一列的 **`[选取]` 按钮** —— 在 Project 里选中并 Ping 这个材质的文件；如果是**内嵌在 Prefab / FBX 里的材质**（`AssetDatabase.GetAssetPath` 返回宿主资产），则定位到宿主资产，否则 Project 里根本没有它的条目。按钮和勾选框一样走手动命中（不做成 `GUI.Button`），因为行是虚拟化的，滚动时控件 ID 会漂移。
