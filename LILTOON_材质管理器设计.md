...
- **中栏下半（日志控制台）**：改动记录 + 诊断合并在一块，标题条右端放原来的选中摘要（工具条里那串数字已挪到这里）。见 §4.4。
...
- 位置：**中栏下半区**（材质表下方），标题条可折叠，右端显示选中摘要（`已选 12 / 材质 128 · 物体 42 · 1.3 ms`）。原来的"本次改动"块从右栏搬到这里，并吸收"诊断"。
...
- 选中变化时也记一条影响面：`选中 12 个材质，共 30 个使用点，其中 4 个来自 Prefab 实例…`。
