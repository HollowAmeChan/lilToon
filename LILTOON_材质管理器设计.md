...
│     ☑ Skirt       │────────────────────────────────│ ── 阴影 ──              │
│     ☐ Coat        │ ▾ 日志                         │   …（分组可折叠，      │
│                   │   [清空] 改动即时生效，Ctrl+Z  │      右栏本身不再折叠） │
│                   │   12:03 _AODarkStrength 0.5→0.7 ×11                     │
│                   │   12:03 ⚠ 跳过 1 个（没有该属性）：Cloth             │
└───────────────────┴────────────────────────────────┴─────────────────────────┘
（材质表与日志之间是横向分隔条，可上下拖动；选中情况/影响面在右键菜单里看）
```
...
- **中栏下半（日志控制台）**：改动记录 + 诊断合并在一块，独立可拖（横向分隔条调高度，双击复位，高度记进 `EditorPrefs`）。选中情况/影响面收进右键菜单，标题条只写"日志"。见 §4.4。
...
- 位置：**中栏下半区**（材质表下方），是**独立可拖的区域**：材质表与日志之间有一条横向分隔条，上下拖动改日志高度，**双击复位**，高度记进 `EditorPrefs`（下次开窗保持）。标题条点一下可折叠（折叠时只剩标题条）。
- 标题条上**只写"日志"**，不再显示摘要 —— 选中情况、影响面都收进右键菜单（见下），界面更干净。
...
- 只做展示，不参与写入；`[清空]` 只清列表。写入仍然是即时的，撤销交给 `Ctrl+Z`（`MaterialProperty` 赋值自带 Undo）。
