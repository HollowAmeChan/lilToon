...
//   _LiquidWaveAmpMul   乘数，脚本每帧写（静止时给 0）
//   最终 = _LiquidWaveAmp * _LiquidWaveAmpMul
//
// 为什么用乘法而不是加法：
//   乘法天然带"闸门"语义 —— 手动那一路给 0 就等于把这条通道整个关掉，
//   乘数写多大都不会有波纹。加法做不到这件事（0 + 任何值照样有波纹），
//   而"关掉某个容器的液面波纹"是美术真会做的操作。
//   反过来，乘数给 0 时最终也是 0，与手动的值无关 —— 静止就是静止。
//
// 默认 _LiquidWaveAmpMul = 1：对只用手动参数的现有材质**完全等价于原来的行为**
// （乘 1 是恒等）。脚本一旦开始写它，就接管了"这一帧波纹该多强"的话语权。
float lilLiquidWaveAmplitude()
{
    return _LiquidWaveAmp * _LiquidWaveAmpMul;
}
