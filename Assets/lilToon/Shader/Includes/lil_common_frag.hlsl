...
// Shadow
    ...
    void lilGetShading(inout lilFragData fd LIL_SAMP_IN_FUNC(samp))
    {
        if(_UseShadow)
        {
            ...
            float shadowReceiveMask = 1.0;
            #if defined(LIL_FEATURE_ShadowReceiveMask)
            ...
            // ShadowBorderMask branch (unsaturate + one final saturate) was equivalent
            // to this one; with AO now folded into aoVis there is only one path left.
            lns.w = lns.x;
...
void lilPlanarReflection(inout lilFragData fd)
{
    ...
    // Resolve the same material inputs published by MetadataBuffer Target5.
    ...
    float3 specular = lerp(_Reflectance, fd.albedo, metallic);
    float oneMinusReflectivity = LIL_DIELECTRIC_SPECULAR.a * (1.0 - metallic);
    ...
    float3 reflectionSource = LIL_SAMPLE_2D_LOD(
        _LILPBRPlanarReflectionTexture,
        lil_sampler_linear_clamp,
        planarUV,
        perceptualRoughness * max(_LILPBRPlanarReflectionParams.w, 0.0)).rgb;
    float3 contribution = reflectionSource * _PlanarReflectionTint.rgb * surfaceReduction * fresnel;

    // PLR is an additive indirect specular contribution, not a replacement color.
    fd.col.rgb += contribution * reflectionWeight;
}
#endif
