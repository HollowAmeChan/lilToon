...
- 本轮继续执行 P3 / P4：先删除 VRC Light Volumes，再删除 LTCGI。
...
- 停止 `red.sim.lightvolumes` 包触发 `LILTOON_VRCLIGHTVOLUMES`。
- 删除 shader setting 中 `LIL_OPTIMIZE_USE_VRCLIGHTVOLUMES` 及 UI 开关。
- 删除随包 `VRC Light Volumes` include。
- 清理 forward / fur / openlit 中的 `VRC_LIGHT_VOLUMES_INCLUDED` 分支。
...
- 停止 `at.pimaker.ltcgi` 包触发 `LILTOON_LTCGI`。
- 删除启动阶段对 CurrentRP 中 LTCGI 状态的读写和变更触发。
- 删除 SubShader tag 注入。
- 删除 HLSL 第三方 include、callback 和 `lilLTCGI` 调用链。
