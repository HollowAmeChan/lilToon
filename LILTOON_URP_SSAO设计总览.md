...
当前 SSAO 插入点（v8：一份共用输入 + 两个输出）：

```hlsl
// lilCalcAO()：每像素合成一次（本体 pass 在 fd.albedo 之后 / BEFORE_SHADOW 之前）
float3 aoVis = 1.0;
#if defined(LIL_FEATURE_ShadowBorderMask)   // 共用颜色贴图
...
// 输出 2：toon ramp 偏移（lilGetShading 内，toon 分级之前）
